...
- ~~History detail/preview now covers current script and metadata JSON, but secondary convenience
  ...
  buttons to the History detail panel, wired to `handle_history_open_folder` and
  `handle_history_copy_path` with cross-platform clipboard and file-explorer support.
