...
4. Before editing any UI control, trace the full path:
   component declaration -> handler/helper -> `.click()` / `.change()` binding.
...
1. `DEFAULT_LLM_NARRATION_SYSTEM_PROMPT`
2. `LLM_PROVIDER_CONFIGS`
3. `apply_llm_narration_transform`
4. narration transform UI controls in `create_gradio_interface`
5. `generate_unified_tts_wrapped` input ordering
